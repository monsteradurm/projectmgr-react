...
import { SetSelectedLog, SetSelectedSheet, SheetRibbonColor, TimesheetData, useLogContextMenu, useSelectedLog, useSelectedSheet, useSheetContextMenu, useTimesheetArtist, useTimesheets, useTimesheetSubmissions } from "./Timesheet.context";
...
const ArtistTemplate = (log) => {
    ...
    return(
        <Stack direction="horizontal" gap={3}>
            <SupportUsers artists={[log.artist]} id={log.id} searchKey="Artist"
...
export const TimesheetSubmissions = ({}) => {
    ...
    const Timesheets = useTimesheetSubmissions();
    ...
    useEffect(() => {
        ...
        if (sub !== submitterFilter)
            setSubmitterFilter(sub);
    ...
    useEffect(() => {
        ...
        if (projectFilter?.length || feedbackFilter?.length || departmentFilter?.length || approversFilter?.length || submitterFilter?.length) {
    ...
    useEffect(() => {
        let titles = ['Home', 'Timesheets', 'Submissions'];
        SetTitles(titles);
    }, [])
    ...
    return (
        ...
        <Stack direction="vertical">
            ...
            <DataTable value={logs} className="pm-timesheet" style={{paddingTop: 0}} scrollable scrollHeight="calc(100vh - 250px)" 
                ...
                <Column header="Artist" body={ArtistTemplate} className="log-artists" style={{width: 70, maxWidth: 70}}></Column>
            ...
            <Stack direction="horizontal" gap={1} className="pm-tag-filters"
                ...
                <div>Artists, </div>
                <div style={{marginLeft: 10, fontWeight: 600}}>{projectCount}</div>
                <div>Projects, </div>
