    ...
    static ItemsSubscriptionHandler = combineLatest(
            ...
            ProjectObservables.ItemsSubscription = 
                FirebaseService.ItemsChanged$(projectId, boardId, groupId).pipe(
                    ...
                    map(items =>
                        items.map(i => ({...i, subitems: i.subitems ? _.sortBy(
                            i.subitems, i => i.Index?.text).filter(
                            (i) => 
                    ...
                    map(items => items.map(i => ({...i, CurrentReview: i.subitems.length > 0 ?
                        _.first(i.subitems) : null})))
                )
