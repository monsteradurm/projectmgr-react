...
import { useSyncsketchReviewName, useUploadReviewDlg, ShowUploadReviewDialog, useCurrentUploadStepIndex, useUploadSyncsketchReview, SetUploadSyncsketchReview, useUploadEvent, ShowAddtoReviewDialog, useAddToReviewDlg, useIsAddFilesUploading, useAddFileUploadEvent, useFilesForUpload, useCopyToReviewDlg, showCopyToReviewDlgEvent$, ShowCopytoReviewDialog } from "./TableItem.Upload.context";
...
const UPLOAD_QID = '/TableCopyToReview'

export const TableItemCopyToReview = ({}) => {
    const BusyMessage = useBusyMessage(UPLOAD_QID);
    const dialogRef = useRef();
    const {BoardItemId, CurrentReviewId, ReviewItems, FeedbackDepartment} = useCopyToReviewDlg();
    ...
    useLayoutEffect(() => {
        if ([ssProjectGroup, ssProject, Element, FeedbackDepartment].indexOf(SUSPENSE) < 0)
            RemoveQueueMessage(UPLOAD_QID, 'init-reviews');
    }, [ssProjectGroup, ssProject, Element, FeedbackDepartment])
    ...
    const header = (
        <DialogHeader color={Status?.color} Header={Task ? Element + ", " + Task : Element}
            HeaderLeft="Copy To Review:" HeaderRight={FeedbackDepartment} onClose={() => ShowCopytoReviewDialog(null)}/>
    )
    ...
    return (
        ...
        className="pm-dialog" ref={dialogRef} onHide={() => ShowCopytoReviewDialog(null)}>
            <ScrollPanel className="pm" style={{ overflowX: 'hidden',
                background: 'white'}}>
                {
                    ...
                    <>
                        ...
                        {
                            <div style={{width: '100%', textAlign: 'center', marginTop: 50, fontWeight: 600}}> 
                                In Progress... </div>
                        }
                        </div>
                    </>
